function setup() {
  let markovModel = createModel(trainingText);

  const NUM_GENERATED_WORDS = 200; // We want to generate 200 words
  let latestWord = "One"; // Start with this word, then overwrite the variable
  let generatedText = [latestWord];
  for (let i = 0; i < NUM_GENERATED_WORDS; i++) {
    let possibleNextWords = markovModel[latestWord];
    if (possibleNextWords !== undefined) {
      latestWord = random(possibleNextWords);
      generatedText.push(latestWord);
    } else {
  ...
  // Put generatedText into the paragraph element
  let generatedTextElement = select("#generatedTextElement");
  generatedTextElement.html(generatedText);
}

function createModel(trainingText) {
  let markovModel = {};
  let arrayOfWords = trainingText.split(" ");
  for (let i = 0; i < arrayOfWords.length - 1; i++) {
    let word = arrayOfWords[i];
    let nextWord = arrayOfWords[i + 1];
    if (markovModel[word] !== undefined) {
      markovModel[word].push(nextWord);
    } else {
      markovModel[word] = [nextWord];
    }
  }
  return (markovModel);
}
